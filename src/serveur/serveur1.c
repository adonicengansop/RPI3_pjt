...
int main(int argc, char *argv[])
{
    ...
    // Définition argument du serveur
    if (argc < 2)
    {
    ...
    information_server.sin_port = htons(portno);
    ...
    while (1)
    {
        ...
        if (pid == 0)
        {
            ...
            memset(msg, 0, 255);
            sprintf(msg,"**********************************\n*\t bienvenu! client %i\t*\n*********************************\n\n 1.Demande prise une photo (Saisir 1)",id);
            ...
            do
            {
                ...
                printf ("client %d : %s\n",id,msg);
                printf ("Réponse : ");
                fgets(msgS, 255, stdin);
                msgS[strlen(msgS) - 1] = '\0';
                send(connexion, msgS, strlen(msgS), 0);
